document.addEventListener("DOMContentLoaded", function () {
  ...
  addButtons.forEach(function (button, index) {
    button.addEventListener("click", function (event) {
      ...
      const saladBox = document.querySelectorAll(".box")[index];
      const saladName = saladBox.querySelector("h3").textContent;
      const saladPrice = parseFloat(saladBox.querySelector(".mon").textContent.replace("pkr ", ""));
  ...
  function updateCartStatus() {
    if (cart.length > 0) {
      ...
      cart.forEach(function (salad, index) {
        const listItem = document.createElement("li");
        listItem.innerHTML = `
          ${salad.name} - ${salad.price.toFixed(2)}
  ...
  function generatePDF() {
    const cartDetailsHTML = `
      ...
      <ul>
        ${cart.map(salad => `<li>${salad.name} - ${salad.price.toFixed(2)}</li>`).join('')}
      </ul>
