document.addEventListener("DOMContentLoaded", function () {
  ...
  addButtons.forEach(function (button, index) {
    button.addEventListener("click", function (event) {
      ...
      const pizzaBox = document.querySelectorAll(".box")[index];
      const pizzaName = pizzaBox.querySelector("h3").textContent;
      const pizzaPrice = parseFloat(pizzaBox.querySelector(".mon").textContent.replace("pkr ", ""));
  ...
  function generatePDF() {
    const cartDetailsHTML = `
      ...
      <ul>
        ${cart.map(pizza => `<li>${pizza.name} - ${pizza.price.toFixed(2)}</li>`).join('')}
      </ul>
