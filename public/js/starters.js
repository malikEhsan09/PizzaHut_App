document.addEventListener("DOMContentLoaded", function () {
  ...
  addButtons.forEach(function (button, index) {
    button.addEventListener("click", function (event) {
      ...
      const starterBox = document.querySelectorAll(".box")[index];
      const starterName = starterBox.querySelector("h3").textContent;
      const starterPrice = parseFloat(starterBox.querySelector(".mon").textContent.replace("pkr ", ""));
  ...
  function updateCartStatus() {
    if (cart.length > 0) {
      ...
      cart.forEach(function (starter, index) {
        const listItem = document.createElement("li");
        listItem.innerHTML = `
          ${starter.name} - ${starter.price.toFixed(2)}
  ...
  function generatePDF() {
    const cartDetailsHTML = `
      ...
      <ul>
        ${cart.map(starter => `<li>${starter.name} - ${starter.price.toFixed(2)}</li>`).join('')}
      </ul>
