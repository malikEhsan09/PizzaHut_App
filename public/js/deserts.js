document.addEventListener("DOMContentLoaded", function () {
  ...
  addButtons.forEach(function (button, index) {
    button.addEventListener("click", function (event) {
      ...
      const dessertBox = document.querySelectorAll(".box")[index];
      const dessertName = dessertBox.querySelector("h3").textContent;
      const dessertPrice = parseFloat(dessertBox.querySelector(".mon").textContent.replace("pkr ", ""));
  ...
  function updateCartStatus() {
    if (cart.length > 0) {
      ...
      cart.forEach(function (dessert, index) {
        const listItem = document.createElement("li");
        listItem.innerHTML = `
          ${dessert.name} - ${dessert.price.toFixed(2)}
  ...
  function generatePDF() {
    const cartDetailsHTML = `
      ...
      <ul>
        ${cart.map(dessert => `<li>${dessert.name} - ${dessert.price.toFixed(2)}</li>`).join('')}
      </ul>
